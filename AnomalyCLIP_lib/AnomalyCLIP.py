...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, gamma, num_head, alpha):
        super().__init__()
    ...
    @torch.no_grad()
    def normalization(self, input, method):
        if method=="norm":
            return input / input.norm(dim=-1, keepdim=True)
        elif method == "sk":
            for _ in range(3):
                input = input / (input.sum(dim=1, keepdim=True) + 1e-8)
                input = input / (input.sum(dim=0, keepdim=True) + 1e-8)
            return input

    @torch.no_grad()
    def visual_embedding(self, x, features_list, DPAM_layer, ffn):
        ...
        for patch_token in patch_tokens:
            ...
            DPAM_token_list.append(patch_token)
        DPAM_token_list = DPAM_token_list[0]
        V, N, D = DPAM_token_list[:-1].shape
        D_h = D // self.num_head
        Tag = F.normalize(DPAM_token_list[:-1], dim=-1)
        Sor = F.normalize(DPAM_token_list[-1:], dim=-1)
        Tag_heads = Tag.view(V, N, self.num_head, D_h)
        Sor_heads = Sor.view(1, N, self.num_head, D_h)
        out = []
        for v in range(V):
            heads_out = []
            for h in range(self.num_head):
                Tag_d3 = Tag_heads[v, :, h, :]
                Sor_d2 = Sor_heads[0, :, h, :]
                dist = ((Tag_d3.unsqueeze(1) - Sor_d2.unsqueeze(0)) ** 2).sum(-1)
                K = torch.exp(- self.gamma * dist)
                K = self.normalization(K, "sk")
                A = F.softmax(K / (D_h ** 0.5), dim=-1)
                fused = A @ Sor_d2
                heads_out.append(fused)
            heads_out = torch.cat(heads_out, dim=-1)
            g = torch.sigmoid(Sor.mean(dim=1))
            heads_out = Tag[v] + g * heads_out
            out.append(heads_out)
        out = torch.stack(out, dim=0)
        return [self.normalization(self.ln_post(x_ori.permute(1, 0, 2)[:, 0, :]) @ self.proj, "norm")[:-1], self.normalization(self.ln_post(x_ori.permute(1, 0, 2)[:, 0, :]) @ self.proj, "norm")[-1:], DPAM_token_list[-1:]], (1 - self.alpha) * Tag + self.alpha * out
    ...
    def preprocessing(self, x):
        ...
        return self.ln_pre(x).permute(1, 0, 2)

    @torch.no_grad()
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        return self.visual_embedding(self.preprocessing(x), features_list, DPAM_layer = DPAM_layer, ffn = ffn)
