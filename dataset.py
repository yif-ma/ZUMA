...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for filename in sorted(os.listdir(d2_render_img_path)):
            ...
            img = self.transform[0](img) if self.transform is not None else img
        ...
        if self.dataset_name == 'mvtec_pc_3d_rgb' or self.dataset_name == 'eye_pc_3d_rgb':
            ...
            return {'img': image, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': anomaly, 'd2_render_img': d2_render_img, 'd2_render_anomaly': rendering_anomaly, 'd2_render_gt': d2_render_gt, 'd2_3d_cor': d2_3d_cor,
                    'img_path': os.path.join(self.root, img_path), "cls_id":self.class_name_map_class_id[cls_name], "d2_render_img_path": d2_render_img_path, "non_zero_index": non_zero_index_arr, 'cmpl': cmpl, "index":torch.LongTensor([index])}
